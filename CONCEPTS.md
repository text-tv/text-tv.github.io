...
**Frame** — the picture of one sub-page as the upstream service publishes it: a fixed-size image, never text. Everything the reader sees on a page originates as a frame, so anything the app wants to know about a page's content it must recover from one. It is exactly as wide as the character grid; the box the app draws it into is deliberately not — see the flagged ambiguity at the end of this file.
...
**Prefetch** — fetching the pages either side of the one being read before the reader asks for them, so a sideways drag reveals a drawn sheet rather than a loading one. It reaches two pages forward and one back: reading runs forward, so a second swipe that way is the one worth being ready for, while the page behind the reader is the one they just left and is already in hand. The second page forward is named by the first one's payload rather than by the page being read, because a page's neighbours are named by that page's own payload and nothing else knows them, so it is asked for as soon as that first neighbour lands. A prefetch is a convenience and never costs the reader a page they visited: it is dropped rather than allowed to evict a stored page, and one that fails is discarded rather than kept as an answer about the page. It may also satisfy itself from a page already stored, but only while that copy is fresh enough to still be servable when the reader arrives — the **arrival test**, which asks not whether a copy is fresh now but whether it will still be fresh when the reader actually gets there, and allows a margin because the reader chooses when to swipe. A copy that fails the test is still drawn straight away and asked for again behind, so that landing on the page costs nothing.
...
- "Frame" names two boxes that are not the same width. The **frame** is the picture the upstream service publishes, exactly as wide as the character grid. The **drawn frame** is the box the app lays that picture into, which is one cell wider. The broadcast leaves the grid's first column blank on the rows carrying full-width banners but runs those banners flush to the far edge, so a picture drawn edge to edge sits a cell off-centre; the app adds a blank column of its own on the opposite side to balance it. The extra column is the app's, never the broadcast's, and the picture stays flush against the near edge of the box rather than centred in it. Say which frame is meant wherever a width is at stake — the difference is exactly one cell, and it is deliberate.
