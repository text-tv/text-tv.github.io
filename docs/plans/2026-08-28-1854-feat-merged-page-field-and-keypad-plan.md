...
- R25. Any control that navigates or refreshes and is not the field — a rail link, prev, next, home, refresh, or a hotspot tap — ends editing and closes the keypad, discarding the typed digits. It fires on the control, not on the page number changing: home while already on 100, or a rail link to the page being read, changes no page, and a keypad left standing there would hold digits the reader can no longer commit.
...
- R26. The field takes focus when the keypad opens and holds it for the whole session. An on-screen key press does not move focus, and the keys are outside the tab order; keyboard readers use R20's direct path instead. Focus is taken explicitly rather than left to the click, because not every engine focuses a button on click.
...
- KTD4. **`editing` lives in `App`; `BottomBar` takes it as a prop and reports changes through `onEditing`.** Three consumers need it — the pull guard (R21), the dock's slide (R18), and the field itself — and two of them are outside `BottomBar`, so a copy in each would be one fact in two places. The digits typed stay in `BottomBar`, which is the only thing that reads them. Because `App` owns it, every control that navigates or refreshes can put the keypad away directly (R25) rather than waiting on a page number that may not change. `BottomBar` clears the digits and any pending commit beat from one effect on `editing`, so no caller can close the keypad and leave either behind. Governs R18, R21, R24, R25.
...
| U6 | The field dims only past the commit distance, only toward a neighbour that exists, and undims on every gesture end, with the commit decision itself unchanged. |
