...
- `docs/solutions/runtime-errors/a-reload-of-a-hash-entry-leaves-chrome-for-ios-sized-to-the-screen-not-the-slot.md` — the same class with a different axis: the environment is the defect, the code is correct, and only real hardware reproduces it. There the environment is Chrome for iOS's own view sizing rather than a display profile, and the page's own measurements all agree while being wrong — the counterpart to reading pixels back and finding them one step off.
