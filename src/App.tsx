...
import { Diagnostics, diagnosticsWanted } from "./components/Diagnostics";
...
export function App() {
  ...
  /** Read once, as `motion` is: the readout is asked for in the URL or not. */
  const [diagnostics] = useState(diagnosticsWanted);
  ...
  useSwipeNavigation({
    ...
    onSwap: (direction) =>
      setSlot(
        (slot) =>
          (slot + (direction === "next" ? 1 : SHEETS.length - 1)) %
          SHEETS.length,
      ),
  ...
  const sheetAt = (offset: number) =>
    SHEETS[(slot + offset + SHEETS.length) % SHEETS.length];
      ...
      <main className="content" ref={content}>
        ...
        */}
        <div className="pull-track" ref={pullTrack}>
          ...
          */}
          <div
            className={`pull-strip pull-strip--${pullState}`}
            aria-hidden="true"
          >
            {/*
            ...
            */}
            {pullState === "fetching" && (
              <span className="pull-strip__spinner">↻</span>
            )}
            <span className="pull-strip__label">
              {PULL_LABEL[pullState](fetchingPage)}
            </span>
            <span className="pull-strip__fill" ref={pullFill} />
          </div>
          <div
            className={
              dragging ? "swipe-track swipe-track--dragging" : "swipe-track"
            }
            ref={track}
            style={
              {
                "--swipe-gutter": `${SWIPE_GUTTER_PX}px`,
